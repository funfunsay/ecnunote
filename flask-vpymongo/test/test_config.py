...
class FlaskVPyMongoConfigTest(util.FlaskRequestTest):
    ...
    def test_default_config_prefix(self):
        self.app.config['MONGO_DBNAME'] = 'flask_vpymongo_test_db'
        self.app.config['MONGO_HOST'] = 'localhost'
        self.app.config['MONGO_PORT'] = 27017

        mongo = flask.ext.vpymongo.PyMongoV(self.app)
        assert mongo.db.name == 'flask_vpymongo_test_db', 'wrong dbname: %s' % mongo.db.name
        assert mongo.cx.host == 'localhost'
        assert mongo.cx.port == 27017

    def test_custom_config_prefix(self):
        self.app.config['CUSTOM_DBNAME'] = 'flask_vpymongo_test_db'
        self.app.config['CUSTOM_HOST'] = 'localhost'
        self.app.config['CUSTOM_PORT'] = 27017

        mongo = flask.ext.vpymongo.PyMongoV(self.app, 'CUSTOM')
        assert mongo.db.name == 'flask_vpymongo_test_db', 'wrong dbname: %s' % mongo.db.name
        assert mongo.cx.host == 'localhost'
        assert mongo.cx.port == 27017
    ...
    def test_rejects_invalid_strint(self):
        ...
        self.assertRaises(TypeError, flask.ext.vpymongo.PyMongoV, self.app)

    def test_sets_after_request(self):
        flask.ext.vpymongo.PyMongoV(self.app)
    ...
    def test_multiple_pymongos(self):
        ...
        for prefix in ('ONE', 'TWO'):
            flask.ext.vpymongo.PyMongoV(self.app, config_prefix=prefix)
